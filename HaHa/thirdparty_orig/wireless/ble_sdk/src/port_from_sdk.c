...
void gpio1_combined_isr_handler(void)
{
	...
	do {
		...
		if (intstatus != 0) {
			...
			/* chris.choi : i think that port_int_callback is always null but if not, then it makes error because
...
enum port_status_code wakeup_int_unregister_callback(enum port_wakeup_source wakeup_source)
{
	...
	if ((wakeup_source == PORT_WAKEUP_SOURCE_AON_GPIO_0) || (wakeup_source == PORT_WAKEUP_SOURCE_AON_GPIO_1)
